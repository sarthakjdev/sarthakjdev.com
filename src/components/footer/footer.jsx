...
export default function Footer(){
    return (
        <footer className="flex bottom-0 justify-around items-center text-white my-6 lg:flex-col lg:my-2 lg:gap-4">
            ...
            <div className="flex items-center justify-around gap-8">
                {
                    socials.map((social)=>{
                        return (
                            <Link href={social.url} key={social.name} ><a target="_blank" ><Image src={social.icon.src} alt={social.name} height={social.icon.height} width={social.icon.width} /></a></Link>
                        )
                    })
                }
            </div>
