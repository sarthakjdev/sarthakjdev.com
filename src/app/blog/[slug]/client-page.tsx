...
import { getAbsolutePostUrl, getPostUrl } from '~/reusable-functions'
...
export default function BlogPostClientPage({ post, morePosts, publication }: Props) {
	...
	const memoisedContent = useMemo(() => imageReplacer(post.content.html, true), [post.content])
		...
		const handleScroll = () => {
			...
			for (const id of headingIds) {
				...
				if (element) {
					...
					console.log('DOM does not have required heading right now')
				}
			}
							...
							<div className="col-span-full mx-auto flex w-full flex-col gap-8">
								...
								<div className="flex flex-col gap-3 border-b-2 border-gray-200/50">
									...
									<div className="flex flex-col items-center justify-center gap-2">
										<div
											className={clsx(
												`font-heading break-words px-4 text-center text-2xl font-extrabold text-slate-900 md:px-5 md:text-4xl lg:px-8 xl:px-10 xl:text-4xl dark:text-white`
											)}
										>
										...
										{post.subtitle && (
											<div className="font-heading px-4 text-center md:px-5 lg:px-8 xl:px-10">
												<h2 className="text-xl leading-snug text-slate-700 md:text-3xl xl:text-2xl dark:text-slate-400">
													{post.subtitle}
												</h2>
											</div>
										)}
								...
								<div id="post-content-parent" className="relative mb-10 pb-14">
									<div
										...
										className="min-h-30 prose prose-lg mx-auto mb-10 break-words xl:prose-xl dark:prose-dark"
						...
						<div className="flex flex-col gap-2">
							{morePosts?.edges.map((edge: any) => {
								...
								return (
									<Link
										key={relatedBlog.id}
										className="hover:bg-secondary-200/40 rounded-md p-1 pl-4 pr-0"
										href={getPostUrl(relatedBlog.slug as string)}
									>
										<div className="flex flex-col items-start gap-1">
											<h2 className="text-secondary-700 text-sm ">
												{relatedBlog.title}
											</h2>
											<div className="flex gap-3">
												...
												<h3 className="text-secondary-500 text-sm">
